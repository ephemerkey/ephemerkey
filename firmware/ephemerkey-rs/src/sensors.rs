...
//!
//! PB3 = INT1 (wake-on-motion, EXTI3). The 2nd alarm channel (INT2) was dropped
//! to free PA8 for PERI_EN, so tamper/free-fall rides INT1 (LIS3DH CTRL_REG3
//! I1_IA2) or the sample loop, not a dedicated pin.
//!
...
//! Motion (an INT1 edge or a supra-threshold sample delta) resets the timer.
...
use embassy_stm32::peripherals::{EXTI3, PB3};
...
pub async fn task(
    ...
    // LIS3DH INT1 push-pull active-high by default config.
    let mut int1 = ExtiInput::new(int1, exti3, Pull::Down, Irqs);

    let mut prev: Option<[i8; 3]> = None;
    let mut still_ms: u32 = 0;
    loop {
        match select(Timer::after_millis(SAMPLE_MS), int1.wait_for_rising_edge()).await {
            ...
            // INT1 fired (motion wake, or tamper/free-fall via I1_IA2) — not still.
            Either::Second(()) => {
                info!("lis3dh: INT1 (motion / tamper)");
                still_ms = 0;
